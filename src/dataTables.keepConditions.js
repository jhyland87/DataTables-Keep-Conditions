 ...
 *  -   Look into storing the values to any input elements that may reside in the table, not 
 ...
 *      CONDITION           NAME            KEY     STATUS
 *      Pagination          page            p       Completed
 *      Page Length         length          l       Completed
 *      Table Search/Filter search          f       Completed
 *      Column Sorting      order           o       Completed
 *      Scroller Extension  scroller        s       Completed
 *      Column Visibility   colvis          v       Refactoring
 *      Colunm Order        colorder        c       Refactoring
 *      Row Selection       rowselect       e       Refactoring
 *      Column Selection    colselect       ?       Not Started
 *      KeyTable            keytable        k       Not Started
 *      Child Row           children        h       Not Started
 *      Row Order           roworder        r       Not Started
 *      
 * Numeric Sequence Hash String Explained
 *      Since this plugin stores the table state in the URL (currently in the hash, 
 *      but adding some functionality that can optionally store data in the URL query 
 *      string), I think that keeping the URL as short as possible. One method I put
 *      in place for this would be to encode any numeric sequences into a 
 *      not-so-user-friendly string, which lmay look like gibberish, but its very useful 
 *      for storing numeric sequences (which is used for ColReorder, RowReorder, 
 *      ColVisibility, RowVisibility, RowSelection, ColumnSelection, etc)
 *
 * Sequence Hash Explained
 *      There's two main "tricks" I use for shortening the hash - use number ranges (in either 
 *      direction), and the other is something I devised myself, which uses a delimiter (a pipe) 
 *      to segregates the numbers of the same length into separate sections, then segregate the 
 *      numbers with the same first N characters (I guess a prefix?) into different sub-sections, 
 *      while placing the numeric "prefix" as the first part of the subsection (to the left of the 
 *      period). If no prefix (meaning no period) is found, then 
 * 
 * Generate shuffled array of a numerical range
 *      php -r '$seq = range(1,20); shuffle($seq); echo join(",",$seq).PHP_EOL;'
 *
 * Basic Examples:
 *      Numeric Sequence (33 chars)  
 *          1,3,4,5,7,9,12,13,14,15,19,20,25
 *
 *      Sequence Blob (19 chars)
 *          13-579|1.2-59|2.05
 *
 *      Numeric Sequence 1-20 (51 chars) 
 *          8,4,3,13,18,15,2,1,17,12,19,5,20,14,9,16,6,11,10,7
 *
c = comma delimited
p = partition (split following number every N characters)

l2.12752365 = 12,75,23,65

 *      Sequence Blob (? chars)
 *          843|2,1.385|21|2,1.729|5|2,2014c9,16,6,11,10,7
 *
 *      Numeric Sequence 1-15 (36 chars) 
 *          10,1,5,2,13,12,9,3,14,4,6,15,8,7,11
 *
 *      Sequence Blob (? chars)
 *          
 *
 ...
 *      Numeric Sequence (98 chars)  
 *          1,3,5,6,7,8,10,11,12,14,16,21,23,24,25,30,31,32,34,36,42,44,45,46,102,103,104,
 *          107,200,202,203,204
 *
 *      Sequence Blob (52 chars)
 *          135-8|1.0-246:2.13-5:3.0-246:4.24-6|10.2-47:20.02-4
 *
 * @example
...
class KeepConditions {
    ...
    conditions ( con ) {
        ...
        var conditions = {
             ...
             * Row Selection 
             *
             * Row selection by either the rows().index(), or the rows().id() if the rowId config is set 
             */
            select: {
                key: 'e', 
